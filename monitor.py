...
class SourceChangeMonitor(threading.Thread):
	...
	ROOT_DIRECTORY = r"c:/Users/hlynu/Desktop/pixel stuff"

	# Entry point program to run                                                                                         
	PROGRAM = r"renderer.py"
	...
	def get_files(self):
		...
		for filename, mtime in files:
			if filename.rsplit('\\',1)[-1] != self.this_script_name.rsplit('\\',1)[-1]:
				file_list.append((filename, mtime))
		#files = filter(lambda filename, mtime: filename.rsplit('\\',1)[-1] != self.this_script_name.rsplit('\\',1)[-1], files)
		return file_list
